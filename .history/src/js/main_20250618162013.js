...
window.addEventListener("scroll", () => {
    ...
    const scale = Math.max(0.5, 1 - window.scrollY / shrinkPoint);
    const translateY = Math.min(50, window.scrollY / 3);
    const translateX = Math.min(50, window.scrollY / 3);

    title.style.transform = `scale(${scale}) translateY(-${translateY}px) translateX(-${translateX}px)`;
});
